...
def get_activity_ids(page, max_page, get_max, name, pwd):
    ...
    while page <= max_page:
        ...
        hrefs = []
        for element in href:
            hrefs.append(str(element.get_attribute("value")).replace("//try.jd.com/", "").replace(".html", ""))
            ids.append(str(element.get_attribute("value")).replace("//try.jd.com/", "").replace(".html", ""))
        hrefs.sort()
...
get_activity_ids(1, 2, 1, "jd账号", "jd密码")
# browser.close()
